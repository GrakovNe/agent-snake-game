...
Единственное запрещённое средство — гамильтонов цикл.

Если проложить по полю
...
статический цикл, и его динамическую починку.

Разрешён любой честный поиск по
...
в среднем **150 клеток из 900**.

Змейка растёт, начинает мешать сама себе
...
Но полных побед на больших полях ноль, и дальше начинается
...
Чем меньше свободных клеток, тем плотнее карусель — в
...
Первая надежда была: подождём нужной фазы.

Мы даже построили точный перебор всех фаз ожидания (в замкнутой петле
«подождать N тактов» — это просто повернуть расписание на N). Перебор оказался почти бесполезен, и причина
...
стенок дырки не меняются никогда.
...
Метка нормируется удвоенной стороной поля (см. листинг), чтобы один
...
превзойти учителя; она лишь удешевляет его.

Больше данных того же сорта учат
...
каждый виток поднимает и учителя, и ученика.

Как выяснится ниже, этот цикл мы прошли до конца — и очередной виток
...
класс решений, и оставался один выход.
...
бывают соседями, финальный ход невозможен.

У нас еда случайна, поэтому
...
инструмент с обратной стороны доски: «фею еды».

Зерно внутренних монеток
...
Существует ли честный способ у самого игрока — открытый
...
кирпичи для этой задачи.

Любое продвижение здесь было бы математическим
