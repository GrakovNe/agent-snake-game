...
и плюс-минус одна ничего не решает.) Написан по горячим следам нескольких
исследовательских суток: со всеми тупиками, опровергнутыми гипотезами и цифрами.
...
Когда змейка занимает ~99% поля, она движется вплотную за собственным
хвостом, без единой клетки зазора: клетку, которую хвост освобождает,
голова тут же занимает. Тело
...
освободилась другая стенка, через которую голова выйдет наружу. Всё решает расписание: каждая клетка
...
95–99%. (Забегая вперёд: «кривая решённости» ниже уточнит это наблюдение —
судьба определяется не в середине партии, а именно в этой поздней зоне; но
в момент выбора очередного пути к еде менять её уже поздно.) Одношаговые
...
## Нейросеть: дорогое обучение, дешёвая игра
...
обучение, но не сама игра. Это ровно схема AlphaZero, применённая в миниатюре: дорогой поиск
...
этим числом средний результат многих доигрываний. Метки собираются
...
181 тысячи примеров (облачная машина размечала их со скоростью ~1090
...
каждый виток поднимает и учителя, и ученика. Забегая вперёд: этот цикл мы
прошли до конца — и очередной виток упёрся в тот же потолок, о чём ниже.
...
## Почему конструкция бота сложилась именно такой
...
## Ночь науки: серия опровержений и одна кривая

За ночь автономной работы шесть гипотез — от «судьба решается в середине
игры» до сверхточных парных оценок — прошли проверку сериями по 200-400 игр,
и почти все умерли, что само по себе результат. Три главных итога:
...
точный оценщик. Первый закрылся быстро: жёсткие ленточные маршруты и
следование заранее выбранной форме стабильно проигрывали хаотичному
перепланированию (подробности — в лабораторном журнале). Второй получил
полноценную проверку — TD-разметка, она же
fitted value iteration. Идея: раз доигрывания шумят (разброс 3-5 клеток),
не будем доигрывать. Новую метку состояния строим одним точным шагом:
...
случайность будущего усреднена явно, а не насэмплирована.
...
корпус на 350 тысяч, на нём обучена новая сеть. Результат: на 30×30 — паритет со всеми
предшественниками, на 60×60 решающий замер в 200 игр — 4 полных партии (2.0%) против
7 из 300 у закреплённой базы (2.3%). Порог прорыва — правило проекта: перед каждым решающим замером мы заранее
...
уже седьмая по счёту.

Одна деталь всё же примечательная: в этой серии случился первый полностью
выигранный матч на 60×60 — все 3600 клеток, строго больше нашей зачётной
планки «минус одна», и без гамильтонова цикла. Трактуем
...
минут вместо двадцати пяти. На ней обучили сеть в десять раз больше прежней.
...
буквально нечего. Выбор формы выжидательных кругов по той же метрике — минус двадцать два
процентных пункта доли полных партий (62 из 200 против 84 у чемпиона, поле 30×30): во время выжидания дыры не рождаются, там нечем управлять.
...
(штатный лимит — два лапа поля: 1800 тактов на малом, 7200 на большом).
Медиана не сдвинулась ни на клетку нигде. Полных партий на малом поле стало
больше на пять из двухсот (84 → 89), на большом — на одну (5 → 6); насыщение
наступает уже на десятикратном бюджете, дальше цифры не растут. А голодных
...
доказанная раньше теоремой, подтвердилась грубой силой. Голод — секундомер
...
(Graafsma, Manthey, Skopalik, июнь 2025). Их результаты отрезвляют.
...
для нас это не приговор, а карта худших случаев. Мы проверили чётность как
рычаг на своих данных: мертва — двудольность решётки задаёт чётность фаз
принудительно, это константа позиции, а не переменная выбора.
...
где статистика набирается быстро). Корреляция исходов между ними оказалась
...
дальнейшую последовательность еды; партии расходятся целиком. Главное же:
поодиночке каждый доводит до полного поля ~40% партий, но **хотя бы один
из трёх — 70%**. Плато, в которое упёрлись все методы, — не потолок достижимого.
Большинство партий выигрываемы кем-то из близнецов; невозможно лишь
угадать заранее, которым. Смена политики действует на исход конкретной
партии как ещё одна монетка — а девять опровержений выше объясняют, почему
...
приходится всё более длинный кусок концовки. Технически каждая попытка —
...
заполнения. Направленно управлять этой фрагментацией умеет только генератор еды:
шестьдесят согласованных появлений еды превращают поражение в победу.
У игрока направленного рычага нет — выбор хода внутри политики вырожден
(кандидаты неразличимы), а смена политики целиком лишь бросает ещё одну
монетку, не сдвигая средний результат. Честные несколько процентов
полных партий — это вероятность того, что равномерные кости сами
сложатся в низкофрагментированную линию.

Для инженерного проекта это редкий итог: потолок не предположен, а огорожен
измерениями со всех сторон — сверху обучением,
...
| + эпизодный поиск (доигрывания) | ~41% | **первые полные партии: 2–7% по разным сериям** |
| + нейросеть вместо доигрываний | ~42%, втрое дешевле | тот же уровень, вдвое дешевле (поле больше — доля поиска в цене хода ниже), без единого примера этого размера |
