...
Этот документ — история погони за целью, которая так и не была взята: научить
бота играть в классическую змейку так, чтобы он стабильно съедал поле 60×60
целиком, все 3600 клеток. Спойлер честности: стабильно — не вышло; вышло
понять и измерить, почему. Потолок оказался интереснее рекорда. (В замерах мы
...
Документ разбит на четыре части: **игра и первые ступени** (правила и то,
что даётся легко), **машинерия** (поиск, нейросеть и почему они устроены
...
воспроизводит старую в точности. Поле W×H без внутренних стен; еда появляется в случайной свободной
клетке, всегда ровно одна. Смертей три: врезаться в стену, врезаться в
собственное тело — и умереть от голода. Голод — полноценное правило игры:
...
любой замер терял бы смысл. Дальше это правило ещё сыграет свою роль — и
как причина половины смертей, и как подозреваемый, которого пришлось
оправдать отдельным экспериментом. Съел последнюю свободную клетку — победа.
...
где появляется еда. Фея даёт честному
...
монетку, не сдвигая средний результат. Честные несколько процентов полных партий на большом поле (и ~40% на
