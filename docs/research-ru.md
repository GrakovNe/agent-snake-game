...
целиком, все 3600 клеток. Скажем сразу: стабильно — не вышло; вышло
...
Выбор архитектуры не был вкусовым. Оценивать нужно форму петли, а её
...
Входные признаки выбраны теорией, а не перебором. Карта расписания
...
дешёвый контекст. Метка нормируется стороной поля, чтобы один масштаб
ошибки годился всем размерам; из всех эпох обучения сохраняется та, что
...
label  = (area - mean_score) / (2 * side)         # дефицит, нормирован стороной
```
...
арендуется прерываемая облачная машина на 96 ядер, ~30–40 ₽/час (прерываемые
...
Шесть гипотез — от «судьба решается в середине игры» до сверхточных парных
...
fun value(state): Double = memo(state) {
    candidates(state).maxOf { walk ->            // путь выбираем мы
        spawns(after(state, walk)).map { food -> // еду выбирает случай
            value(after(state, walk, food))
        }.average()
    }
}
// бот исполняет только линии с value == 1.0 — сертифицированные победы
```
...
while (best.status != WON && replays < budget) {
    ...
    backOff *= 2                                    // упорным неудачам — глубже откат
}
```
...
Потолок здесь не предположен, а огорожен измерениями со всех сторон — сверху обучением,
...
Слепая фея перебрасывает кости, пока не повезёт, — на большом поле это
стоило девять часов работы 88 ядер ради пяти побед. Направленная будет
выбирать место появления еды по измеренному профилю удачи: ближе к голове,
с минимальной фрагментацией после съедения. Это одновременно и инструмент,
и эксперимент: если направленный выбор начнёт выигрывать на порядок дешевле
слепого, причинная цепочка «еда → фрагментация → судьба» подтвердится уже
...
перестраивал его при каждом появлении еды. Наивные версии — жёсткие ленты,
